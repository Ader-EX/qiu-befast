...
async def getAllTOP(db : Session = Depends(get_db),
    ...
    if from_date and to_date:
        query = query.filter(
            TermOfPayment.created_at.between(
                datetime.combine(from_date, time.min),
                datetime.combine(to_date, time.max),
            )
        )
    elif from_date:
        query = query.filter(TermOfPayment.created_at >= datetime.combine(from_date, time.min))
    elif to_date:
        query = query.filter(TermOfPayment.created_at <= datetime.combine(to_date, time.max))
